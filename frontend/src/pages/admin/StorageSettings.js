...
import { Cloud, ExternalLink, CheckCircle, AlertCircle, HelpCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-serif text-3xl mb-2">Cloud Storage</h1>
      ...
      <div className={`gem-card p-4 mb-6 flex items-center gap-3 ${settings.cloud_storage_enabled ? 'border-green-500/30' : 'border-yellow-500/30'}`}>
        {settings.cloud_storage_enabled ? (
          ...
          <>
            ...
            <span className="text-yellow-400">Using direct URLs only</span>
          </>
        )}
      </div>
      ...
      <div className="gem-card p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="font-medium">Enable Cloud Storage</p>
            <p className="text-xs text-gray-500">Upload and manage media from admin panel</p>
          </div>
          <button
            onClick={() => setSettings(prev => ({ ...prev, cloud_storage_enabled: !prev.cloud_storage_enabled }))}
            className={`w-12 h-6 rounded-full transition-colors ${settings.cloud_storage_enabled ? 'bg-green-500' : 'bg-gray-700'}`}
          >
            <div className={`w-5 h-5 rounded-full bg-white transition-transform ${settings.cloud_storage_enabled ? 'translate-x-6' : 'translate-x-0.5'}`} />
          </button>
        </div>
